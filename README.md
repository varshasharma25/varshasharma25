# Building. Breaking. Learning. Securing.

### Hey there, I'm Varsha Sharma 

A curious engineer exploring Cybersecurity, IoT, AI, and the endless possibilities of technology.

From research papers and hackathons to community leadership and open-source learning, I'm on a mission to build secure, meaningful, and impactful solutions.

Welcome to my corner of the internet.


---

## About Me

🔐 Exploring Cybersecurity and Ethical Hacking

🌐 Building IoT and Smart Systems

🤖 Interested in Artificial Intelligence and Emerging Technologies

🏆 Active Hackathon Participant

📄 Researcher in AI & IoT Applications

🌱 Lifelong Learner with a Curiosity-Driven Mindset

---

## Current Focus

* Cybersecurity Fundamentals & Network Security
* Secure Software Development
* IoT & Embedded Systems
* AI-Powered Solutions
* Open Source Contributions
* Research & Innovation

---

## Tech Stack

### Languages

```text
C • JavaScript • HTML • CSS
```

### Technologies & Tools

```text
Arduino • Flask • PostgreSQL • Git • GitHub
```

### Currently Learning

```text
Linux • Networking • Web Security • Cloud Technologies
```

---

## Research

### Smart Women Safety System Using IoT and Artificial Intelligence

Research focused on leveraging IoT devices and AI-driven intelligence to enhance personal safety through real-time monitoring, alert systems, and smart decision-making.

---

## Leadership & Soft Skills

✨ Leadership

✨ Community Building

✨ Public Speaking

✨ Technical Communication

✨ Problem Solving

✨ Team Collaboration

✨ Critical Thinking

As a Google Student Ambassador 2026, I actively engage in student communities, organize initiatives, and help create opportunities for fellow learners to grow through technology.

---

## GitHub Analytics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=varshasharma25&show_icons=true" alt="GitHub Stats" />
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=varshasharma25 alt="GitHub Streak" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=varshaSharma25&layout=compact" alt="Top Languages" />
</p>

---

## Contribution Graph

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=varshasharma25" alt="Contribution Graph" />
</p>

---

## What You'll Find Here

* Personal Projects
* IoT Experiments
* Cybersecurity Learning Journey
* Research Work
* Hackathon Projects
* Technical Notes & Documentation
* Open Source Contributions

---

## Beyond Technology

I enjoy learning in public, collaborating with ambitious people, and taking on challenges that push me outside my comfort zone.

Whether it's a hackathon, a research project, an open-source contribution, or a community initiative, I believe every experience is an opportunity to learn and create impact.

> "Technology is most powerful when it is secure, accessible, and capable of solving real-world problems."

---

## Connect With Me

🤝 Open to collaborations, hackathons, research opportunities, open-source projects, and meaningful tech discussions.

---



### Building • Learning • Securing • Leading
